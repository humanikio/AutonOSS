...
interface AttendeeSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectContact: (attendee: AttendeeInput) => void;
}

export default function AttendeeSelectionModal({
  isOpen,
  onClose,
  onSelectContact
  ...
  // Get contacts for attendee selection with pagination and search
  const {
    contacts,
    loading,
    error,
        ...
        {contacts.length > 0 && (
          <div className="p-4 border-t border-gray-200 bg-gray-50/50">
            <p className="text-xs text-gray-500 text-center">
              Click on a contact to add them as an attendee
            </p>
          </div>
        )}
      </div>
