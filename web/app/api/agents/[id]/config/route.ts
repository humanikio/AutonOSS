...
export async function PATCH(
  ...
  try {
    ...
    const { elevenLabsAgentId, updates } = body;
    ...
    if (!updates || Object.keys(updates).length === 0) {
      return NextResponse.json(
        { success: false, error: 'Configuration updates are required' },
        { status: 400 }
      );
    }
    ...
    const response = await fetch(`${backendUrl}/api/agent-management/agents/${agentId}/config`, {
      method: 'PATCH',
      headers: {
