...
import { Wifi, AlertCircle, Plus, Trash2 } from 'lucide-react';
...
interface TriggerNodeData {
  ...
  categoryColor?: string; // Hex color from category (e.g., '#EAB308')
}
...
const TriggerNode = ({ data, selected, id }: TriggerNodeProps) => {
  // Defensive check: if no label, node data is corrupted
  if (!data?.label) {
    console.error('TriggerNode: Missing label in node data', { id, data });
    return (
  ...
  // Get category color or use default orange
  const categoryColor = data.categoryColor || '#F59E0B';
  ...
  return (
    ...
    >
      ...
      {/* Right Handle - Output (Drag to Connect) */}
      <Handle
        type="source"
