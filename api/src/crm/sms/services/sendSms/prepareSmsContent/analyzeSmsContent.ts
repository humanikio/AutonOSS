export interface AnalyzeSmsContentRequest {
  content: string;
}

export interface AnalyzeSmsContentResponse {
  variables: string[];  // Unique variable names found in template
}

class AnalyzeSmsContent {
  /**
   * Extracts custom variables from SMS template
   ...
   */
  analyze(request: AnalyzeSmsContentRequest): AnalyzeSmsContentResponse {
    console.log(`=🔍 Analyzing SMS content`);
...
export const analyzeSmsContent = new AnalyzeSmsContent();
