...
/**
 * Resolves custom variable names to their actual values from contact data
 *
 * Fetches contact data and maps variable names to values using the
 * flattened contact format for easy key-value access.
 *
 * Uses the same resolution logic as SMS/Email/Workflow systems.
 */

export interface ResolveVariablesRequest {
  tenantId: string;
  contactId: string;
  variableNames: string[];  // Variable names extracted from prompt
}
