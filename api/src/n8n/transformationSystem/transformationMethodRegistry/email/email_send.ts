...
export class SendEmailTransformation implements Transformation {
  readonly name = 'email_send';
  readonly priority = 50;

  matches(node: ReactFlowNode, config?: INodeTypeDescription): boolean {
    return config?.name === 'pulselineSendEmail';
  }
  ...
  transform(node: ReactFlowNode, config: INodeTypeDescription, context: TransformationContext): TransformationResult {
    ...
    const endpoint = config._pulseline?.apiEndpoint || '/api/email/send';

    // Build body object from node parameters
    const bodyObj: Record<string, any> = {
      contactId: nodeParams.contactId || '{{$contact.contactId}}',
      emailAccountId: nodeParams.emailAccountId,
      to: nodeParams.to || '{{$contact.email}}',
      subject: nodeParams.subject,
    };

    // Handle conditional template vs message
    if (nodeParams.contentType === 'template' && nodeParams.templateId) {
      bodyObj.templateId = nodeParams.templateId;
    } else {
      bodyObj.message = nodeParams.message;
    }
...
export const email_send = new SendEmailTransformation();
