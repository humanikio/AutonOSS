/**
 * Opportunity Update Transformation
 * Transforms Pulseline Update Opportunity node to HTTP Request node
...
export class OpportunityUpdateTransformation implements Transformation {
  readonly name = 'opportunity_update';
  readonly priority = 50;

  matches(node: ReactFlowNode, config?: INodeTypeDescription): boolean {
    return config?.name === 'pulselineUpdateOpportunity';
  }
...
export const opportunity_update = new OpportunityUpdateTransformation();
