...
export class DeleteContactTransformation implements Transformation {
  readonly name = 'contact_delete';
  readonly priority = 50;

  matches(node: ReactFlowNode, config?: INodeTypeDescription): boolean {
    return config?.name === 'pulselineDeleteContact';
  }
  ...
  transform(node: ReactFlowNode, config: INodeTypeDescription, context: TransformationContext): TransformationResult {
    ...
    const endpoint = config._pulseline?.apiEndpoint || '/api/contacts/{{$parameter["contactId"]}}';
    ...
    while ((match = paramRegex.exec(endpoint)) !== null) {
      ...
      if (userValue !== undefined && userValue !== '') {
        ...
        if (strValue.startsWith('={{') || strValue.startsWith('{{')) {
          ...
          // It's a literal value - add as-is with quotes
    ...
    const httpParameters: HttpRequestParameters = {
      ...
      url: fullUrl,
      authentication: 'none',
      sendBody: false, // DELETE doesn't need a body
      sendHeaders: true,
      specifyHeaders: 'json',
    ...
    return {
      ...
      metadata: {
        ...
        nodesCreated: 1,
        custom: { endpoint, contactId: nodeParams.contactId },
      },
    };
  }
}

export const contact_delete = new DeleteContactTransformation();
