...
export class UpdateContactTransformation implements Transformation {
  readonly name = 'contact_update';
  readonly priority = 50;

  matches(node: ReactFlowNode, config?: INodeTypeDescription): boolean {
    return config?.name === 'pulselineUpdateContact';
  }
  ...
  transform(node: ReactFlowNode, config: INodeTypeDescription, context: TransformationContext): TransformationResult {
    ...
    const endpoint = config._pulseline?.apiEndpoint || '/api/contacts/{{$parameter["contactId"]}}/manage';
...
export const contact_update = new UpdateContactTransformation();
