/**
 * Event Brain - Generate Handler JSONs (Stage 3 - LLM Call #2)
...
export async function generateHandlerJsons(
  ...
  console.log(`\n🧠 Event Brain - Stage 3: Generating Handler JSONs`);
...
- Find eventId/taskId for semantic updates/deletes
- Detect conflicts with existing items
- Link new events to related tasks
- Avoid creating duplicates
- Match existing event patterns (for recurring events)
...
2. Use calendar context (if provided) to find IDs, detect conflicts, or link items
3. Use the detailed handler specifications to understand parameters
4. Convert all relative times using CURRENT DATE & TIME context (ALL TIMES IN UTC!)
5. Generate descriptive event names (not generic)
6. Choose appropriate eventType for each event
7. Ensure startTime and endTime are both present and valid
8. Output the exact number of handler calls specified in operations
9. Include ALL required parameters for each handler
...
`;
    ...
    if (parsed.handlerCalls.length === 0) {
      ...
      // Return with empty handlerCalls - eventService will post message and exit gracefully
      return parsed;
    }
