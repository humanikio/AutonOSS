/**
 * Event Brain - Pull Context (Stage 2 - NO LLM)
    ...
    if (!calendarId) {
      ...
      try {
        ...
        const calendarContextData = await fetchCalendarContext(tenantId, calendarId, {
          ...
          includeEvents: reviewOutput.contextRequest.includeEvents ?? true,
          taskFilters: reviewOutput.contextRequest.taskFilters as any
        });
...
`);
  ...
  for (const operation of reviewOutput.operations) {
    ...
    switch (handler) {
      case 'create':
        console.log(`    Loading createEventHandler context`);
        handlerContexts.push(CREATE_EVENT_CONTEXT);
        break;

      case 'update':
        console.log(`    Loading updateEventHandler context`);
        handlerContexts.push(UPDATE_EVENT_CONTEXT);
        break;

      case 'delete':
        console.log(`    Loading deleteEventHandler context`);
        handlerContexts.push(DELETE_EVENT_CONTEXT);
        break;

      case 'read':
        console.log(`    Loading readEventHandler context`);
        handlerContexts.push(READ_EVENT_CONTEXT);
        break;

      case 'getEvents':
        console.log(`    Loading getEventsHandler context`);
        handlerContexts.push(GET_EVENTS_CONTEXT);
        break;
